...
state/               volatile runtime signals; gitignored
  <id>.status        appended by crewmates as optional-UTC-timestamp plus "<state>: <note>" wake-event lines, not current-state truth
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .wedge-absorb-since-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
CI and pipeline configuration (`.woodpecker/`, `.github/workflows/`, and shared CI scripts) is shared mutable state that always serializes to exactly one owner lane; feature lanes never carry their own copy of an in-flight CI change.
Experiments and side work get a backlog record at dispatch, and the muster sweep surfaces unpushed work rather than leaving it to be discovered by forensics.
...
Run `bin/fm-pr-check.sh <id> <PR url> [--intended-base <branch>]` - it records `pr=`, the forge's `pr_head=` when available, and optional `intended_base=` in the task's meta before arming the watcher's merge poll.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `MUSTER:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `NUDGE_SECONDMATES:`, `FMX:`, `BETTER_STACK:`, `WOODPECKER_ERRORS:`, or `DISK_FLOOR:`); silence and `BOOTSTRAP_INFO:` need no load.
...
- `just-say-no-to-process-porn-and-ceremony` - load at session start before work or dispatch, and reload before a relied-on done claim, a release after gate changes, or when process-only activity looks busy; the skill owns the full doctrine and `fm-brief.sh` injects its canonical credit-rules module into every new ship, scout, and secondmate instruction.
