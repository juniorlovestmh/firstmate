...
#                 "WOODPECKER_ERRORS: monitoring on ..." or
#                 "WOODPECKER_ERRORS: monitoring off ...",
#                 "DISK_FLOOR: <free>GiB free on <volume> is below the
#                 <floor>GiB floor - reclaim with: bin/fm-disk-reclaim.sh --apply".
#          DISK_FLOOR is a read-only detect line (bin/fm-disk-lib.sh) printed even
#          in lock-refused read-only mode; it never blocks bootstrap itself, only
#          bin/fm-spawn.sh refuses. config/disk-floor overrides the default 10GiB
#          floor (docs/configuration.md "Disk floor and reclaim").
...
#          x_mode_setup, better_stack_incidents_setup, woodpecker_errors_setup,
#          disk_guard_check_setup, fleet_sync) while still printing every
#          read-only detect line
#          above; the TANGLE line switches to advisory-only wording with no
#          checkout command. Used by
#          fm-session-start.sh's read-only path when another live session holds
#          the fleet lock, so a second concurrent session never race-mutates
#          PR-check artifacts, secondmate homes, X-mode, Better Stack, or
#          Woodpecker poll artifacts, project clones, disk guard registration, or repair
#          instructions.
...
# Woodpecker creation-error monitoring is an explicit home-local opt-in.
# The poll owns pipeline and diagnostic dedupe because ordinary custom-check
# output travels through the watcher's generic authenticated check-wake path.
woodpecker_errors_setup() {
  local flag check trust check_body tool missing check_home failed
  flag="$CONFIG/woodpecker-error-poll"
  check="$STATE/woodpecker-errors.check.sh"
  trust="$STATE/woodpecker-errors.check-trust"

  woodpecker_errors_remove_artifacts() {
    local remove_failed=0
    x_mode_remove_artifact "$check" || remove_failed=1
    x_mode_remove_artifact "$trust" || remove_failed=1
    [ "$remove_failed" -eq 0 ]
  ...
  if [ ! -e "$flag" ] && [ ! -L "$flag" ]; then
    if x_mode_artifact_present "$check" || x_mode_artifact_present "$trust"; then
      if woodpecker_errors_remove_artifacts; then
        echo "WOODPECKER_ERRORS: monitoring off - removed the home-scoped poll"
      else
        echo "WOODPECKER_ERRORS: monitoring off - failed to remove the home-scoped poll"
      fi
    fi
    return 0
  fi
  if [ ! -f "$flag" ] || [ -L "$flag" ] || [ "$(fm_pr_file_link_count "$flag")" != 1 ]; then
    woodpecker_errors_remove_artifacts || true
    echo "WOODPECKER_ERRORS: monitoring off - config/woodpecker-error-poll must be an ordinary file"
    return 0
  fi
  ...
  if [ "$missing" -ne 0 ]; then
    woodpecker_errors_remove_artifacts || true
    echo "WOODPECKER_ERRORS: monitoring off - install the reported poll dependencies and rerun bootstrap"
    return 0
  fi
  ...
  if [ "$failed" -eq 0 ]; then
    check_body=$(printf '%s\n' \
      '#!/usr/bin/env bash' \
      '# Auto-generated by fm-bootstrap.sh - Woodpecker error custom check.' \
      '# Registered bytes call the tracked poll; output becomes a check: wake.' \
      "export FM_HOME=$(printf '%q' "$check_home")" \
      "exec $(printf '%q' "$FM_ROOT/bin/fm-woodpecker-errors-poll.sh")")
    bootstrap_write_if_changed "$check" "$check_body" 700 || failed=1
  fi
  if [ "$failed" -eq 0 ] && ! fm_custom_check_registered "$STATE" woodpecker-errors; then
    FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" FM_ROOT_OVERRIDE="$FM_ROOT" \
      "$SCRIPT_DIR/fm-check-register.sh" woodpecker-errors >/dev/null 2>&1 || failed=1
  fi
  if [ "$failed" -eq 0 ]; then
    fm_custom_check_registered "$STATE" woodpecker-errors || failed=1
  fi
  if [ "$failed" -ne 0 ]; then
    if woodpecker_errors_remove_artifacts; then
      echo "WOODPECKER_ERRORS: monitoring off - failed to arm the home-scoped poll"
    else
      echo "WOODPECKER_ERRORS: monitoring off - failed to arm the home-scoped poll; stale artifacts remain"
    fi
    return 0
  fi

  echo "WOODPECKER_ERRORS: monitoring on - registered state/woodpecker-errors.check.sh at the default 300s check cadence"
}

# Disk guard (fleet-local-disk-guard-o14) is a durable, ALWAYS-ON registered
# check - unlike Better Stack it needs no external credential or opt-in flag,
# so every locked mutating sweep arms it unconditionally. Below-floor
# detection itself lives in bin/fm-disk-lib.sh (shared with bin/fm-spawn.sh's
# refusal); this only wires that check into the watcher's existing
# registered-check extension point exactly like better_stack_incidents_setup,
# with no bespoke watcher handling - the generic "check: $c: $out" path in
# fm-watch.sh is the only consumer.
disk_guard_check_setup() {
  local check check_home check_body failed
  check="$STATE/disk-guard.check.sh"
  ...
  if [ "$failed" -eq 0 ]; then
    check_body=$(printf '%s\n' \
      '#!/usr/bin/env bash' \
      '# Auto-generated by fm-bootstrap.sh - disk guard registered check.' \
      '# Registered bytes call the tracked poll; output becomes a check: wake.' \
      "export FM_HOME=$(printf '%q' "$check_home")" \
      "exec $(printf '%q' "$FM_ROOT/bin/fm-disk-guard-poll.sh")")
    bootstrap_write_if_changed "$check" "$check_body" 700 || failed=1
  fi
  if [ "$failed" -eq 0 ] && ! fm_custom_check_registered "$STATE" disk-guard; then
    FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" FM_ROOT_OVERRIDE="$FM_ROOT" \
      "$SCRIPT_DIR/fm-check-register.sh" disk-guard >/dev/null 2>&1 || failed=1
  fi
  if [ "$failed" -eq 0 ]; then
    fm_custom_check_registered "$STATE" disk-guard || failed=1
  fi
  if [ "$failed" -ne 0 ]; then
    echo "DISK_FLOOR: failed to arm the registered disk guard watcher check"
    return 0
  fi
}
...
# reading a read-only digest deserves the same warning as a locked one. The
# refusal-grade check lives in bin/fm-spawn.sh; this is detection only.
if ! DISK_FLOOR_MSG=$(fm_disk_floor_breach "$CONFIG"); then
  echo "DISK_FLOOR: $DISK_FLOOR_MSG"
fi
crew=
