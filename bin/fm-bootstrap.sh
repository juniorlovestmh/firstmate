...
better_stack_incidents_setup() {
  local flag="$CONFIG/better-stack-incidents" check="$STATE/better-stack-incidents.check.sh"
  local trust="$STATE/better-stack-incidents.check-trust" check_body tool missing=0 check_home failed=0
  ...
  for tool in doppler curl jq; do
    if ! command -v "$tool" >/dev/null 2>&1; then missing_tool_diagnostic "$tool"; missing=1; fi
  done
  if [ "$missing" -ne 0 ]; then
    x_mode_remove_artifact "$check" || true
    x_mode_remove_artifact "$trust" || true
  ...
  if [ "$failed" -eq 0 ]; then
    check_body=$(printf '%s\n' '#!/usr/bin/env bash' '# Auto-generated by fm-bootstrap.sh - Better Stack incident custom check.' "export FM_HOME=$(printf '%q' "$check_home")" "exec $(printf '%q' "$FM_ROOT/bin/fm-better-stack-incidents-poll.sh")")
    x_mode_write_if_changed "$check" "$check_body" 700 || failed=1
  fi
  if [ "$failed" -eq 0 ] && ! fm_custom_check_registered "$STATE" better-stack-incidents; then
    FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" FM_ROOT_OVERRIDE="$FM_ROOT" "$SCRIPT_DIR/fm-check-register.sh" better-stack-incidents >/dev/null 2>&1 || failed=1
  fi
  if [ "$failed" -eq 0 ]; then fm_custom_check_registered "$STATE" better-stack-incidents || failed=1; fi
  if [ "$failed" -ne 0 ]; then
    x_mode_remove_artifact "$check" || true
    x_mode_remove_artifact "$trust" || true
...
woodpecker_errors_setup() {
  local flag="$CONFIG/woodpecker-error-poll" check="$STATE/woodpecker-errors.check.sh"
  local trust="$STATE/woodpecker-errors.check-trust" check_body tool missing=0 check_home failed=0
  if [ ! -e "$flag" ] && [ ! -L "$flag" ]; then
    if x_mode_artifact_present "$check" || x_mode_artifact_present "$trust"; then
      x_mode_remove_artifact "$check" || true
      x_mode_remove_artifact "$trust" || true
  ...
  if [ "$failed" -eq 0 ]; then
    check_body=$(printf '%s\n' '#!/usr/bin/env bash' '# Auto-generated by fm-bootstrap.sh - Woodpecker error custom check.' "export FM_HOME=$(printf '%q' "$check_home")" "exec $(printf '%q' "$FM_ROOT/bin/fm-woodpecker-errors-poll.sh")")
    x_mode_write_if_changed "$check" "$check_body" 700 || failed=1
  fi
  if [ "$failed" -eq 0 ] && ! fm_custom_check_registered "$STATE" woodpecker-errors; then
    FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" FM_ROOT_OVERRIDE="$FM_ROOT" "$SCRIPT_DIR/fm-check-register.sh" woodpecker-errors >/dev/null 2>&1 || failed=1
  fi
  if [ "$failed" -eq 0 ]; then fm_custom_check_registered "$STATE" woodpecker-errors || failed=1; fi
  if [ "$failed" -ne 0 ]; then
    x_mode_remove_artifact "$check" || true
    x_mode_remove_artifact "$trust" || true
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  local_phase && better_stack_incidents_setup
  local_phase && woodpecker_errors_setup
