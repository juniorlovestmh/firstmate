...
| `fm-fleet-inventory-configure.sh` | Validate or deploy the tracked fleet-wide Steampipe GCP connection inventory |
| `fm-fleet-inventory-rotate.sh` | Rotate the fleet-inventory GCP reader key through the project-scoped policy exception |
| `fm-fleet-snapshot.sh`   | Print the read-only structured fleet snapshot JSON (schema `fm-fleet-snapshot.v1`)   |
| `fm-fleet-view.sh`       | Render the fleet snapshot as a human Markdown view                                   |
| `fm-bearings-snapshot.sh` | Project the fleet snapshot to the compact TOON bearings view; local-only unless `--include-prs` |
| `fm-analytics-export.sh` | Write a rebuildable redacted daily analytics snapshot from durable home records only |
...
| `fm-toolchain-mirror.sh` | Snapshot and restore the installed fleet-critical CLI toolchain from a checksummed local offline mirror |
...
| `fm-secrets-check.sh`    | Validate the Doppler standard, project manifests, value-safe inventories, and high-confidence leak rules |
...
| `fm-pr-check.sh`         | Record validated `pr=`, `pr_head=`, and optional `intended_base=` values, then atomically arm a static merge poll |
| `fm-pr-merge.sh`         | Verify a GitHub PR's actual base against its expected base, then record metadata and merge the canonical URL |
