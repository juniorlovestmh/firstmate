...
`bin/fm-supervision-lib.sh` also recognizes X-mode, Better Stack, and Woodpecker home-level polls as supervision needs, but unaccounted primary work still contributes nothing to that predicate.
Without an independent home-monitoring need, unaccounted primary work therefore reads as idle rather than suspicious.
