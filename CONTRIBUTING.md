...
Every generated Firstmate ship, scout, and secondmate instruction also carries the canonical credit-rules module owned by `just-say-no-to-process-porn-and-ceremony`; the scaffold refuses to generate an instruction when that module is missing or empty.
...
bin/fm-test-isolation-proof.sh --list   # proven parallel candidate set (concurrent isolation proof owner)
