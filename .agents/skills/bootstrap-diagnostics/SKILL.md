...
description: >-
  ...
  Use whenever the session-start digest's bootstrap section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, MUSTER, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, PR_CHECK_MIGRATION, SECONDMATE_SYNC, SECONDMATE_LIVENESS, NUDGE_SECONDMATES, FMX, BETTER_STACK, WOODPECKER_ERRORS, or DISK_FLOOR - or when a standalone bin/fm-bootstrap.sh run prints one of those lines.
...
- `MUSTER: <n> worktree(s) holding unrecorded work - run bin/fm-muster.sh` - run the printed read-only muster command, inspect every listed dirty, local-only, or locally-ahead worktree, and make sure each live item has a durable backlog record before cleanup or further dispatch.
...
- `WOODPECKER_ERRORS: monitoring on ...` - the home-scoped poll is registered at the default check cadence; no action is needed.
- `WOODPECKER_ERRORS: monitoring off - removed ...` - the local presence flag was removed and bootstrap retired the runnable check while retaining pipeline and diagnostic dedupe state; no action is needed.
- Any other `WOODPECKER_ERRORS:` line - follow its concrete dependency, unsafe flag, activation, or cleanup diagnostic before relying on Woodpecker error monitoring.
